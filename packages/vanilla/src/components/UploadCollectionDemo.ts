 ...
 *
 * A factory takes data once, so "live" here means calling update() with the new
 * array — this is what a caller's own state layer would do.
 */
function gated(): HTMLElement {
  let items: UploadItem[] = [DONE[0], LIFECYCLE[3]];
  let timer: ReturnType<typeof setTimeout> | undefined;

  const list = UploadCollection({
    items,
    class: "zen-w-full zen-max-w-xl",
    emptyMessage: "Both rows removed — reload to bring them back",
    onRemove: (item) => set(items.filter((i) => i.id !== item.id)),
...
function wired(): HTMLElement {
  ...
  const list = UploadCollection({
    ...
    onRemove: (item) => set(items.filter((i) => i.id !== item.id)),
    onRename: (item, name) => set(items.map((i) => (i.id === item.id ? { ...i, name } : i))),
  });
  function set(next: UploadItem[]) {
    items = next;
    list.update({ items });
...
export default function UploadCollectionDemo(): HTMLElement {
  return DemoPage({
    ...
    description:
      "The list of files that have been uploaded, or are on their way. FileUpload is the input; this is the result.",
    sections: [
      {
        title: "1. A list of attachments",
        codeTitle: "Data in, list out",
        codeDescription:
          "Items default to status 'complete', because a list of files uploaded in some earlier session is the common case — you should not have to stamp every row with a status to render a document library. Give an item a url and its name becomes a link; size, uploadedAt and uploadedBy join one meta line, and whichever you omit is simply absent rather than blank.",
        code: `const items = [
...
UploadCollection({ items }).el`,
        render: () => UploadCollection({ items: DONE, class: "zen-w-full zen-max-w-xl" }).el,
      },
      {
        title: "2. Status and progress",
        codeTitle: "The component renders the upload; it does not run it",
        codeDescription:
          "There is no url prop, no method and no retry policy. You own the transport — fetch, XHR, a resumable chunked client — and write status and progress back onto the item, then call update(). A component that owned the upload would have to guess at your endpoint, headers, auth refresh and chunk size, and every real app would end up fighting it. An uploading item with no percentage says so in words — Uploading… — rather than drawing a bar: Radix renders an indeterminate bar as an empty track and Kobalte renders it as a full one, so the same bar would read 'nothing has happened' in React and 'finished' in Solid. A bar is drawn only when there is a real number behind it.",
...
list.update({ items: next });`,
      ...
      },
      {
        ...
        code: `const list = UploadCollection({ items: [], onRemove, onRename });
const upload = FileUpload({
  multiple: true,
  showFileList: false,
  onValueChange: (files) => list.update({ items: files.map(toItem) }),
});`,
        render: () => wired(),
      },
      {
        ...
        render: () => UploadCollection({ items: THUMBS, class: "zen-w-full zen-max-w-xl" }).el,
      },
      {
        title: "6. Empty",
        ...
        code: `UploadCollection({ items: [], emptyMessage: "No attachments on this order" }).el`,
        render: () =>
          UploadCollection({
            items: [],
