...
/**
 * ShellBar demo — the web-components port. <zen-shell-bar>'s `logo`, `items`,
 * `profile` and `menuItems` are set as JS properties (Node / array / object
 * data). Several sections constrain width, because the two collapses — items into
 * the ••• menu, search into a magnifier — only appear when the bar runs out of room.
 */

interface ShellBarItem {
  id: string;
  label: string;
  icon?: string;
  overflow?: "never" | "auto";
}
interface ShellBarMenuItem {
  id: string;
  label: string;
  icon?: string;
...
interface BarSpec {
  ...
  ariaLabel?: string;
  /** Minimal bars omit the profile. Defaults to showing it. */
  noProfile?: boolean;
}

function bar(spec: BarSpec): HTMLElement {
  const el = document.createElement("zen-shell-bar");
  if (spec.primaryTitle) el.setAttribute("primary-title", spec.primaryTitle);
  if (spec.secondaryTitle) el.setAttribute("secondary-title", spec.secondaryTitle);
  if (spec.searchable) el.setAttribute("searchable", "");
  if (spec.notificationCount != null) el.setAttribute("notification-count", String(spec.notificationCount));
  if (spec.ariaLabel) el.setAttribute("aria-label", spec.ariaLabel);
  Object.assign(el, {
    logo: logo(),
    ...(spec.noProfile ? {} : { profile: PROFILE }),
    ...(spec.items ? { items: spec.items } : {}),
    ...(spec.menuItems ? { menuItems: spec.menuItems } : {}),
  });
  return el;
}
    ...
    sections: [
      {
        ...
        code: `const bar = document.createElement("zen-shell-bar");
bar.setAttribute("primary-title", "Purchase Orders");
bar.setAttribute("secondary-title", "Northwind");
bar.setAttribute("searchable", "");
bar.setAttribute("notification-count", "3");
bar.logo = makeLogo();
bar.items = [{ id: "calendar", label: "Calendar", icon: "calendar" }, /* … */];
bar.profile = { name: "Rajesh Pillai", image: "/me.jpg", menuItems: PROFILE_MENU };
host.append(bar);`,
        render: () =>
          frame(
            null,
            bar({
              ...
              items: ITEMS,
              notificationCount: 3,
              ariaLabel: "Purchase Orders (full width)",
            }),
          ),
      },
      {
        ...
        render: () =>
          frame(
            ...
            bar({
              ...
              ariaLabel: "Purchase Orders (product menu)",
            }),
          ),
      },
      {
...
box.append(bar);   // bar.items has eight entries`,
        render: () =>
          frame(
            700,
            bar({
              ...
              items: MANY_ITEMS,
              notificationCount: 3,
              ariaLabel: "Purchase Orders (700px)",
            }),
          ),
      },
      {
        ...
        render: () =>
          frame(
            ...
            bar({
              ...
              items: ITEMS,
              notificationCount: 3,
              ariaLabel: "Purchase Orders (480px)",
            }),
          ),
      },
      {
        ...
        code: `bar.items = [{ id: "settings", label: "Settings", icon: "cog", overflow: "never" }, /* …rest */];

const minimal = document.createElement("zen-shell-bar");
minimal.setAttribute("primary-title", "Northwind");
minimal.logo = makeLogo();`,
        render: () => {
          ...
          wrap.append(
            frame(
              ...
              bar({
                ...
                ariaLabel: "Purchase Orders (pinned item)",
              }),
            ),
            frame(null, bar({ primaryTitle: "Northwind", noProfile: true, ariaLabel: "Northwind (minimal)" })),
          );
          return wrap;
        },
      },
