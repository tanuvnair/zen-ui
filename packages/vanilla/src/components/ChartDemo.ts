...
/** Wrap a chart so it fills the preview width, matching the React demo. */
const full = (node: Node): HTMLElement => {
...
export default function ChartDemo(): HTMLElement {
  ...
  series: [{ key: "spend", label: "Spend" }, { key: "budget", label: "Budget" }],
        ...
        render: () =>
          full(Chart({ type: "line", data: DATA, xKey: "month", series: SERIES, height: 280 }).el),
      },
      {
        title: "2. Area",
        codeTitle: 'type="area"',
        code: `Chart({ type: "area", data, xKey: "month", series });`,
        render: () =>
          full(Chart({ type: "area", data: DATA, xKey: "month", series: SERIES, height: 280 }).el),
      },
      {
        title: "3. Bar",
        codeTitle: 'type="bar"',
        code: `Chart({ type: "bar", data, xKey: "month", series });`,
        render: () =>
          full(Chart({ type: "bar", data: DATA, xKey: "month", series: SERIES, height: 280 }).el),
      },
      {
        title: "4. Pie",
        codeTitle: 'type="pie"',
        codeDescription:
          "A pie asks parts-of-a-whole rather than value-over-a-range, so every row is a slice — but it needs no new props for that: xKey already names the label and the first series already names the value. A second series would be a second pie, so only the first is read.",
