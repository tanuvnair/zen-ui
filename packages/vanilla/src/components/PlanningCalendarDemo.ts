...
PlanningCalendar({ rows }).el`,
        render: () => PlanningCalendar({ rows: TEAM, now: NOW, defaultDate: NOW }).el,
      },
      {
        ...
        code: `PlanningCalendar({ rows, defaultView: "month" }).el
PlanningCalendar({ rows, views: ["day", "week"] }).el
cal.update({ view: "day" });   // controlled: push the view back in`,
        render: () => PlanningCalendar({ rows: TEAM, now: NOW, defaultDate: NOW, defaultView: "month" }).el,
      },
      {
...
{ id: "x4", start: at(21, 14),     end: at(21, 14) }   // a milestone`,
        render: () => PlanningCalendar({ rows: ONE_DAY, now: NOW, defaultDate: NOW, defaultView: "day" }).el,
      },
      {
  ...
  onAppointmentClick: (appointment, row) => openEditor(appointment, row),
      ...
      },
      {
        ...
        code: `PlanningCalendar({ rows: [], emptyMessage: "No one is scheduled this week" }).el`,
        render: () => PlanningCalendar({ rows: [], now: NOW, defaultDate: NOW, emptyMessage: "No one is scheduled this week" }).el,
      },
    ],
  });
}
