...
// in src/index.css. That makes 1rem = 10px so legacy SCSS can write `1.4rem`
// to mean 14px. UnoCSS preset-uno ships Tailwind's defaults assuming the
// browser default 1rem = 16px, so every rem-based utility (h-*, text-*, p-*,
// gap-*, …) would otherwise render at 62.5% of its intended size. We rescale
// every rem value UnoCSS emits by 16/10 = 1.6, leaving raw rem CSS untouched.
const REM_SCALE = 1.6

export default defineConfig({
  presets: [presetUno()],
  // UnoCSS scans files automatically. This config also wires the design tokens
  // declared in src/styles/tokens.css into the utility theme so classes like
  // `bg-zen-primary`, `text-zen-primary-fg`, `ring-zen-ring` resolve to
  // `var(--zen-color-*)` and consumers can retheme by overriding those vars.
  postprocess: (util) => {
