...
import { Badge, Dialog } from "../index";

/**
 * The footer's version chip and the dialog it opens — the web-components
 * equivalent of the React demo's ReleaseNotes.tsx.
 *
 * Built from zen-ui's own Dialog and Badge (the vanilla factories, re-exported
 * from this package) rather than bespoke markup, for the same reason the React
 * one is built from Radix's: the demo asking people to use these components
 * should be using them, and a panel like this is a fair test of whether they
 * compose. The notes themselves come from @algorisys/zen-ui-core/release-notes —
 * one copy for one design system.
 */
...
export function ReleaseNotes(): { el: HTMLElement; destroy(): void } {
  ...
  chip.type = "button";
  chip.title = `@algorisys/zen-ui-web-components ${ZEN_VERSION} — what's new`;
