...
const NewSelectDialogDemo = () => {
  ...
  const [shortlist, setShortlist] = useState<string[]>(["s2", "s5"]);
  ...
  return (
    <div className="demo-page">
      ...
      <section className="demo-section">
        <h2>1. Single select — the click is the answer</h2>
        <CodeExample
          title="No OK button"
          description="Picking a row confirms and closes. Cancel is the only footer action, because OK would be a second click that says nothing new. The current selection is marked when the dialog re-opens."
...
/>`}
      ...
      <section className="demo-section">
        <h2>2. Multi select — nothing commits until OK</h2>
        <CodeExample
          title="multiple + Clear"
          description="Checkbox rows, and a Clear that is disabled while the draft is empty. Cancel restores whatever was selected when the dialog opened — the ticks you made in between are thrown away, which is the whole point of a draft."
          code={`const [shortlist, setShortlist] = useState<string[]>(["s2", "s5"]);
...
/>`}
      ...
      <section className="demo-section">
        <h2>3. Without search, and the empty state</h2>
        <CodeExample
          title="searchable={false}"
          description="For a short, fixed list a search field is furniture. Type a query that matches nothing in the sections above to see emptyText instead."
          code={`<SelectDialog
  searchable={false}
