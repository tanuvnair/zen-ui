...
import { resolveDateRange, type DateRangeValue } from "@algorisys/zen-ui-core/date-range";
...
/** Named once, so neither signal is seeded by reading the other. */
...
const columns: ColumnDef<Order>[] = [
  ...
  { accessorKey: "city", header: "City" },
  {
    ...
    header: "Status",
    cell: (info) => (
      <Badge variant="soft" color={statusColor(info.row.original.status)}>
  ...
  {
    accessorKey: "amount",
    header: () => <span class="zen-block zen-text-right">Amount</span>,
    cell: (info) => (
      <span class="zen-block zen-text-right zen-tabular-nums">{money(info.row.original.amount)}</span>
    ),
  },
];

const NewListReportDemo = () => {
  // Two signals, not one. `draft` is what the controls hold; `applied` is what
  // the table is showing. Go copies one to the other. That split IS the List
  // Report contract — see section 2.
  const [draft, setDraft] = createSignal<Filters>(INITIAL);
  const [applied, setApplied] = createSignal<Filters>(INITIAL);

  const rows = createMemo(() => ORDERS.filter((o) => matches(o, applied())));
  ...
  return (
    <div class="demo-page">
      ...
      <p class="lede">
        ...
        result, act on a row. It is <code>FilterBar</code> + <code>DataTable</code>{" "}
        and nothing else — no new component, no list-report prop. Both{" "}
        <code>filter-bar.tsx</code> files have said since they were written that
        the gap analysis calls this screen unbuildable without them; this is the
        screen.
      </p>
      ...
      <section class="demo-section">
        ...
        <CodeExample
          ...
          code={`const [draft, setDraft] = createSignal(EMPTY);      // what the controls hold
const [applied, setApplied] = createSignal(EMPTY);  // what the table shows
...
const [draft, setDraft] = createSignal(EMPTY);
...
//                              ^ no arguments — you already have them`}
        >
          <div class="zen-flex zen-flex-col zen-gap-2 zen-text-xs">
            <div>
            ...
            <p class="zen-m-0 zen-text-zen-muted-fg">
