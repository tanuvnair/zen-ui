...
const ReleaseNotes = () => {
  ...
  return (
    <Dialog open={open()} onOpenChange={setOpen}>
      <DialogTrigger
        class="app-version"
        title={`@algorisys/zen-ui-solid ${ZEN_VERSION} — what's new`}
      >
        v{ZEN_VERSION}
      </DialogTrigger>
      <DialogContent class="zen-max-w-2xl">
        <DialogHeader>
        ...
        <div class="zen-max-h-[60vh] zen-overflow-y-auto zen-pr-1">
          <ul class="zen-m-0 zen-flex zen-list-none zen-flex-col zen-gap-4 zen-p-0">
            <For each={notes}>
              {(n) => (
                <li class="zen-flex zen-flex-col zen-gap-1">
                  <div class="zen-flex zen-items-center zen-gap-2">
                    <Badge color={KIND_COLOR[n.kind]} variant="soft">
                      {KIND_LABEL[n.kind]}
                    </Badge>
                    <span class="zen-text-sm zen-font-medium">{n.title}</span>
                  </div>
                  <Show when={n.detail}>
                    <p class="zen-m-0 zen-text-sm zen-text-zen-muted-fg">{n.detail}</p>
                  </Show>
        ...
        <p class="zen-m-0 zen-text-xs zen-text-zen-muted-fg">
          ...
          <Show when={hidden > 0}>
            {`${hidden} older ${hidden === 1 ? "note" : "notes"} not shown. `}
          </Show>
