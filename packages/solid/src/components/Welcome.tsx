...
/**
 * The generated thumbnail for a route, or nothing at all. Mirrors the React
 * binding's CardPreview — see its comment for why the image is a card-sized
 * 1:1 CROP rather than a shrunk page, and why a missing file has to be
 * survivable (they are gitignored and generated by `bun run gen:previews`).
 */
const CardPreview = (props: { route: string }) => {
  const slug = () =>
    props.route === "/" ? "_welcome" : props.route.replace(/^\//, "").replace(/\//g, "-");
  return (
    <img
      ...
      aria-hidden
      loading="lazy"
      class="zen-block zen-w-full zen-border-b zen-border-zen-border zen-bg-zen-background zen-object-cover zen-object-left-top"
              ...
              <div class="zen-grid zen-grid-cols-1 zen-gap-3 sm:zen-grid-cols-2 lg:zen-grid-cols-3">
                <For each={group.items}>
                  {(item) => (
                    <A
                      href={item.path}
                      class="zen-block zen-overflow-hidden zen-rounded-zen-md zen-border zen-border-zen-border zen-bg-zen-background zen-no-underline zen-transition-colors hover:zen-border-zen-primary focus-visible:zen-outline-none focus-visible:zen-ring-2 focus-visible:zen-ring-zen-ring focus-visible:zen-ring-offset-2"
                    >
                      <CardPreview route={item.path} />
                      <div class="zen-p-4">
