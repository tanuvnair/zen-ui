...
export default function RichTextDemo(): HTMLElement {
  ...
  const editor = document.createElement("zen-rich-text");
  editor.setAttribute("value", html);
  editor.setAttribute("placeholder", "Write something…");
  editor.addEventListener("zen-change", (e) => {
    ...
    sections: [
      {
        ...
        codeTitle: "value / zen-change",
        code: `<zen-rich-text value="<p>Hello…</p>" placeholder="Write something…"></zen-rich-text>

editor.addEventListener("zen-change", (e) => { html = e.detail; });`,
        render: () => {
      ...
      },
      {
        ...
        codeTitle: "zen-change output (updates on blur)",
        code: `// html state`,
        render: () => output,
      },
    ],
