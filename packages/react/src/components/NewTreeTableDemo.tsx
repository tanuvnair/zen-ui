...
/>`}
      ...
      <section className="demo-section">
        <h2>7. Pagination pages the ROOTS</h2>
        <CodeExample
          title="`enablePagination` — `pageSize` counts top-level rows"
          description="A page carries each root's whole subtree, so pageSize counts roots and a page's rendered row count varies with what is open. That is the only coherent way to page a tree: paging the flattened list cuts through a subtree and strands its children on the next page under no parent at all. Expand a row below and watch the row count change while the page still holds the same number of top-level rows."
          code={`<TreeTable
  data={data}
  columns={columns}
  enablePagination
...
/>`}
      ...
      <section className="demo-section">
        <h2>8. Virtualization — for a tree you expand all of</h2>
        <CodeExample
...
/>`}
      ...
      <section className="demo-section">
        <h2>9. Keyboard and screen readers</h2>
        <CodeExample
